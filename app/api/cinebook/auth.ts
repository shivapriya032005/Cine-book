...
async function exchangeAuthCode(
  ...
  const resp = await fetch(`${env.cinebookAuthUrl}/api/oauth/token`, {
    method: "POST",
...
const jwks = jose.createRemoteJWKSet(
  new URL(`${env.cinebookAuthUrl}/api/.well-known/jwks.json`),
);
...
export function createOAuthCallbackHandler() {
  return async (c: Context) => {
    ...
    try {
      ...
      if (!userProfile) {
        throw new Error("Failed to fetch user profile from CineBook Open");
      }
